...
`onEventCreate` lifecycle event to open a `popup` after a successful drop when more details need to be collected.
...
`onEventCreateFailed` event, for example when a task is dropped onto an invalid weekend date.
